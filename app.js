...
"   ⚖️ REGLA QUE PIDIÓ REY (06-09): «cada vez que suspendo la laptop el Ejecutor lo anota, pero no sé si soy yo, o ella sola, o si es un error; Roberto me dice que revise y me pregunta SIEMPRE, y no sé qué hacer». NO LE PREGUNTES POR LOS HUECOS QUE YA TIENEN EXPLICACIÓN. Un evento pc_siesta es la FIRMA de una suspensión programada suya (la de las 14:05 al cerrar su ventana operativa, o la de las 06:15 al terminar la killzone de Londres): si una ceguera arranca junto a una firma pc_siesta, dilo en una línea —«de tal a tal hora fue tu siesta programada, nada que revisar»— y SIGUE. Solo se revisa, y solo entonces se le pregunta, la ceguera SIN FIRMA: esa sí puede ser un corte de internet, un reinicio de Windows o una avería, y ahí sí dile a qué hora fue, cuánto duró y si se perdió alguna señal. Una pregunta que se repite siempre deja de significar algo, y entonces el día que de verdad importe no la va a mirar. ⚠️ Y NO TE INVENTES LA CAUSA. Rey te cazó el 06-09 diciéndole «el Ejecutor estuvo ciego 2-3 ratos hoy y ayer (SIN INTERNET)» — y no era eso: el sábado su PC estaba APAGADA a propósito, porque el viernes y el sábado la apaga del todo (es su ritual, lo tienes en tu manual). Tú NO sabes por qué se quedó ciego si el hueco no lleva firma: di la hora, cuánto duró y si se perdió alguna señal, y AHÍ PARA. Poner una causa que no te consta es darle un dato falso, y por un dato falso se pone a buscar una avería que no existe. Si el hueco cae en viernes o sábado, ni lo llames avería: es su ritual.\n"+
...
async function revisionCadena(){
  ...
  /* ⏰ v7.22 — Rey: "sobre los horarios se refiere SOLO a las alarmas del indicador, no a
     los avisos programados; es muy diferente… que los avisos programados se mantengan igual".
     Así está hecho en la nube (el filtro del horario vive solo en la ruta de las alarmas del
     gráfico; tus avisos ⏰, las killzones, las noticias, el Ejecutor y los análisis van por
     otro camino que no lo mira), pero este texto lo decía a medias y podía leerse como si
     el horario callara TODO. Ahora lo dice con las palabras de Rey. */
  L.push("\n🔕 LO QUE NO TE LLEGA A PROPÓSITO (lo pediste tú, pero míralo)");
  L.push("Estas reglas son SOLO LAS ALARMAS DEL INDICADOR. Tus avisos programados (⏰ Mis avisos, killzones, noticias, "
        +"Ejecutor, análisis, buenos días) NO tienen horario ni filtro de tipo: te llegan siempre, a cualquier hora.");
  const hNY = (()=>{ try{
  ...
  L.push("⏰ HORARIO (solo alarmas del indicador): te avisan de 01:00 a 13:00 de Nueva York "
        +"(02:00 a 14:00 tuyas). "+(enHorario?"AHORA MISMO SÍ avisarían.":"AHORA MISMO NO avisarían: estás fuera de esa franja.")
        +" Tus avisos programados no se ven afectados.");
...
/* ══════════════════════════════════════════════════════════════════════════════
   ...
   hacerlo, pero una IA y un sistema sí pudieran hacer lo que yo no puedo".
   Aquí ve lo que el sistema lleva contado. Y si el libro es joven, LO PRIMERO que
   lee es que todavía no se puede sacar ninguna conclusión — porque un porcentaje
   con cuatro operaciones detrás no es un dato, es una trampa que da confianza. */
async function libroSenales(){
  ...
  if(R.cerradas){
    /* ⚠️ LA ADVERTENCIA VA ANTES QUE EL NÚMERO, siempre. Si va debajo, no se lee. */
    if(!R.fiable){
      t+="⚠️ TODAVÍA NO TE FÍES DE ESTOS NÚMEROS\n";
      t+="Con "+R.cerradas+" operacion"+(R.cerradas===1?"":"es")+" no se puede calcular ninguna probabilidad.\n";
      t+="Faltan "+R.faltanParaFiarse+" para poder mirarlos con algo de confianza (y unas 100 para apoyarte en ellos).\n";
      t+="Te los enseño para que veas que el libro funciona, NO para que decidas con ellos.\n\n";
    }
    t+="Esperanza: "+(R.esperanzaR>0?"+":"")+R.esperanzaR+"R por operación\n";
    t+="Aciertos: "+R.aciertoPct+"%\n\n";
    if(R.fiable){
      t+= R.esperanzaR>0
        ? "👉 En positivo: de media ganas "+R.esperanzaR+" veces lo que arriesgas, por operación.\n\n"
        : "👉 En negativo: tal como va, cada operación te cuesta de media "+Math.abs(R.esperanzaR)+" veces lo que arriesgas.\n\n";
      t+="Recuerda lo que de verdad manda: no es el % de aciertos, es la esperanza. Con objetivos de 2R\n"+
         "se gana dinero fallando 6 de cada 10.\n\n";
    }
  } else {
  ...
  S.slice(0,10).forEach(s=>{
    ...
    if(s.desenlace) t+=" → "+s.desenlace+(s.r!=null?" ("+(s.r>0?"+":"")+s.r+"R)":"");
...
async function avisosAlRelojDelTelefono(){
  try{
    ...
    /* 👻 v7.69 — LOS DEL TEMPLO SE CANCELAN SIEMPRE, EXISTA O NO EL PLAN. Rey (07-09) recibió
       a las 06:30 un aviso de entrenamiento sin haber configurado el templo: en Apex no había
       ni plan ni ese aviso, pero en el reloj de Android seguía el despertador de un plan viejo,
       puesto otra vez para el día siguiente. Sus ids son fijos (tplan0, tplan1…), así que se
       mandan a cancelar SIEMPRE: si el plan existe se vuelven a poner tres líneas más abajo, y
       si no existe se van. Un plan borrado no puede seguir despertándole. */
    for (let i = 0; i < 8; i++) if (todos.indexOf("tplan"+i) < 0) todos.push("tplan"+i);
...
function temploHorarioPorDefecto(nSesiones){
  ...
  /* 🕐 v7.68 — LA HORA ES LA QUE ÉL DIJO, no las 18:00 de siempre. Rey (07-09): recibió un
     aviso de entrenamiento a una hora que nunca eligió, porque aquí había un valor fijo. */
  const hora = (TEMPLO_PLAN && /^\d{1,2}:\d{2}$/.test(String(TEMPLO_PLAN.horaBase||""))) ? TEMPLO_PLAN.horaBase : "18:00";
  return dias.map(d=>({ dia:d, hora }));
}
...
function temploHorarioModal(){
  ...
  abrirModal("⏰ Mis horarios de entreno", `
    ...
    <!-- 🗓️ v7.73 — LOS DÍAS QUE PUEDE, TAMBIÉN AQUÍ. Rey (07-09): "habíamos acordado que nada
         ...
         trabajo, entonces pudieran variar por la mañana temprano o por la tarde".
         Tenía razón y faltaba una pieza: los días que puede entrenar solo se preguntaban en la
         PRUEBA DE NIVEL, así que si su trabajo cambiaba y esta semana solo podía tres, tenía
         que rehacer la prueba física para cambiar un número. Ahora se cambia aquí, cuando
         quiera, sin tocar su nivel ni su progreso. -->
    <label class="lbl" style="margin-top:12px">¿Cuántos días puedes entrenar esta temporada?</label>
    <select class="inp" id="thDias">
      ${[2,3,4,5].map(d=>'<option value="'+d+'"'+((parseInt(TEMPLO_PLAN&&TEMPLO_PLAN.dias,10)||4)===d?" selected":"")+'>'+d+' días</option>').join("")}
    </select>
    <div class="note" style="text-align:left">
      Cámbialo cuando cambie tu trabajo. <b>No pierdes tu nivel ni tu progreso</b>: solo se
      ajusta cuántas sesiones te pongo. Si bajas los días, se quedan las primeras.
    </div>
    <div class="note" style="text-align:left;margin-top:10px">
    ...
    [{t:"Cancelar",fn:cerrarModal},{t:"Guardar",cls:"gold",fn:()=>{
      ...
      /* 🗓️ v7.73 — sus días mandan y se guardan ANTES que el horario: si acaba de bajar de 4 a
         3, las sesiones que quedan son 3 y el horario se recorta con ellas. */
...
/* 🧩 v7.68 — ¿le falta a Roberto algo básico para poder acompañarle de verdad?
   El entrenamiento sale de la prueba física, pero TODA la mitad nutricional (calorías,
   proteína, déficit, IMC, progreso de medidas) necesita estos cuatro datos. Sin ellos el
   templo funciona a medias, y eso hay que DECIRLO en vez de disimularlo. */
function temploFaltanDatos(){
  try{
    const f = TEMPLO_FICHA;
    if(!f) return true;
  ...
  try{
    ...
    sem.sesiones.forEach((x,i)=>{
      ...
      REMINDERS.push({
        ...
        tit:"🏋️ "+x.n,
        /* 🧩 v7.68 — SI LE FALTAN SUS DATOS, SE LO DICE EN EL PROPIO AVISO. Rey (07-09):
           "todavía no he configurado el templo —mis medidas, mis datos, mis objetivos— y ya
           me notificó". El plan se crea solo con la prueba física, así que Roberto puede
           empezar a entrenarle sin poder calcularle NADA de la otra mitad (calorías, proteína,
           déficit, progreso de medidas). Antes eso no se decía en ninguna parte: parecía que
           el templo estaba completo. */
        msg:"Hoy te toca: "+cuerpo+". Ábreme y te lo marco."+(temploFaltanDatos() ? " ⚠️ Aún no tengo tus datos (peso, estatura, nacimiento, objetivo): sin ellos no puedo calcularte calorías ni progreso." : ""),
        ir:"tab:templo",
      });
...
function temploPruebaModal(){
  ...
  abrirModal("📏 Prueba de nivel", `
    ...
    <!-- 🕐 v7.68 — SE LE PREGUNTA LA HORA. Rey (07-09): "todavía no he configurado el templo,
         mis medidas, mis datos, mis objetivos, EL HORARIO, LOS DÍAS, y ya me notificó".
         Y tenía razón: el plan se ponía a sí mismo lunes/miércoles/viernes a las 18:00 y
         programaba las alarmas sin preguntarle. Una hora inventada es un aviso que él no
         eligió — y los avisos que no eligió son los que deja de mirar. -->
    <label class="lbl">¿A qué hora sueles entrenar?</label>
    <input class="inp" id="tpHora" type="time" value="${esc((TEMPLO_PLAN && TEMPLO_PLAN.horaBase) || "18:00")}">
    <div class="note" style="text-align:left">
      Los días se reparten solos, con descanso entre medias. <b>Los dos los puedes cambiar
      después</b>, sesión por sesión, en el horario del templo o en ⏰ Mis avisos.
    </div>`,
    [{t:"Cancelar",fn:cerrarModal},{t:"Ver mi nivel",cls:"gold",fn:()=>{
      const m={ ts:Date.now(), dias:parseInt(($("#tpDias")||{}).value,10)||4 };
      const horaBase = /^\d{1,2}:\d{2}$/.test((($("#tpHora")||{}).value)||"") ? $("#tpHora").value : "18:00";
      ...
      TEMPLO_PLAN = { nivel: Math.max(nivel, antes), semanaNum: (TEMPLO_PLAN?TEMPLO_PLAN.semanaNum:0),
                      ...
                      horaBase: horaBase, horario: null };   /* v7.68: su hora, no una inventada */
...
/* 🎤 v7.22 — en la APK no hay dictado de navegador, pero SÍ el de Android: si solo se
   mirara `SR`, el botón se quedaría apagado en el teléfono de Rey (que es donde lo usa). */
function iaMicSoportado(){ try{ return !!SR || !!micNativo(); }catch(_){ return !!SR; } }
function iaMicUI(on){
...
function iaDictarEmpezar(quien){
  try{
    ...
    OIDO.desdeBurbuja = _dictadoDesdeBurbuja;
    if(RECon || OIDO.on) return;           /* ya estaba escuchando: no se reinicia */
    if(oidoEmpezar()) return;              /* 🎤 v7.22: el oído de Android */
    iaEscuchar();
  }catch(_){}
}
...
function iaDictarSoltar(){
  try{
    ...
    if(OIDO.on){ oidoParar(); _dictadoDesdeBurbuja = false; return; }   /* 🎤 v7.22 */
...
/* ══════════════════════════════════════════════════════════════════════════════
   🎤 EL OÍDO DE ROBERTO DENTRO DE LA APK — v7.22 ("la fase de hablarle")
   ═════════════════════════════════════════════════════════════════════════════
   Rey: "podemos continuar… la fase de hablarle: él me responde, con todos los comandos de
   pausarlo, callarlo, etc."

   POR QUÉ ESTABA PUESTO EL BOTÓN Y NO FUNCIONABA: el dictado de arriba es el DEL NAVEGADOR
   (`webkitSpeechRecognition`). En la web va perfecto; dentro de la APK NO EXISTE — el
   navegador interno de Android no trae reconocedor. Así que el 🎤 estaba ahí y al tocarlo
   solo decía "tu teléfono no permite dictado por voz", cuando el teléfono sí puede: había
   que pedírselo a ANDROID, exactamente igual que se hizo con la voz de Roberto en la v6.57.
   ⚠️ ES EL MISMO PATRÓN QUE HA MORDIDO TRES VECES ESTA SEMANA (el botón de Algo Trading que
   el worker tiraba, la prueba de avisos que era muda, y esto): medio camino construido — la
   parte que se VE, puesta; la que hace el trabajo, no. Un botón que existe y no hace nada
   es peor que un botón que falta: manda a buscar la avería donde no está.

   ⚠️ Y NO SE DUPLICA NADA: el camino de la web sigue intacto, letra por letra. Aquí solo se
   elige la puerta — si hay puente de Android, se usa el oído de Android; si no, el de
   siempre. Lo que pasa DESPUÉS (pintar lo dictado y mandárselo a Roberto) es el mismo
   código para los dos.
   ═════════════════════════════════════════════════════════════════════════════ */
...
function micNativo(){
  try{
    const P = vigiaPuente();
    if(!P || typeof P.oirEmpezar !== "function") return null;
    if(!_oidoListo){
      _oidoListo = true;
      try{
...
function oidoPintar(texto, esFinal){
  const ta = $("#iaText");
  const t = String(texto||"").trim();
  if(ta && t){
    ta.value = (OIDO.base + t).replace(/\s{2,}/g," ").replace(/^\s+/,"");
...
/* ══════════════════════════════════════════════════════════════════════════════
   🎛️ LOS COMANDOS DE VOZ — pausarlo, callarlo, seguir
   ═════════════════════════════════════════════════════════════════════════════
   Rey los pidió por su nombre: "él me responde, con todos los comandos de pausarlo,
   callarlo, etc.".
   ⚠️ LA REGLA QUE EVITA EL DESASTRE: una orden solo cuenta si es TODO lo que dijo y son
   POCAS PALABRAS. Si se buscara "para" dentro de la frase, "para qué sirve el CRT de 4
   horas" sería una orden de callarse y Rey se quedaría sin respuesta sin entender por qué.
   Se compara la frase ENTERA, sin tildes ni signos.
   Y NO CUESTAN CRÉDITOS: se resuelven aquí mismo, sin pasar por Roberto. */
const OIDO_ORDENES = [
  { re:/^(callate|calla|silencio|shh+|basta|para|parate|para ya|ya para)$/, hace:()=>{ iaVozParar(); toast("🔇 Callado"); } },
  { re:/^(pausa|pausalo|espera|espera un momento|un momento|para un momento)$/, hace:()=>{ iaVozPausa(); toast("⏸️ En pausa — dime «sigue» cuando quieras"); } },
  { re:/^(sigue|continua|seguimos|dale|adelante|sigue hablando)$/, hace:()=>{ iaVozSeguir(); toast("▶️ Sigo donde iba"); } },
  { re:/^(repite|repitelo|otra vez|di eso otra vez)$/, hace:()=>{ oidoRepetir(); } },
  { re:/^(borra|borralo|olvidalo|cancela|dejalo|nada)$/, hace:()=>{ toast("Borrado"); } },
];

function oidoNormal(t){
  return String(t||"").toLowerCase()
...
/** ¿Lo que dijo es una orden? Si lo es, se hace y se devuelve true (no va a Roberto). */
function oidoOrden(texto){
  const t = oidoNormal(texto);
  if(!t || t.split(" ").length > 4) return false;   /* una orden es corta, siempre */
  for(const o of OIDO_ORDENES){
    if(o.re.test(t)){ try{ o.hace(); }catch(_){} return true; }
  }
  return false;
}

/** Que vuelva a decir lo último que dijo — sin volver a preguntárselo (gratis). */
function oidoRepetir(){
  try{
    const c = iaConvAct();
    for(let i=(c.msgs||[]).length-1; i>=0; i--){
      const m = c.msgs[i];
      if(m && m.role==="assistant" && m.content){ iaVozParar(); iaHablar(m.content, i); return; }
    }
    toast("Todavía no te he dicho nada que repetir");
  }catch(_){ }
}

/** Arranca el oído de Android. Devuelve true si se hizo cargo. */
function oidoEmpezar(){
  const PM = micNativo();
  if(!PM) return false;
...
function oidoParar(){
  const PM = micNativo();
  if(!PM) return false;
  try{ PM.oirParar(); }catch(_){}
  return true;
}

function iaEscuchar(){
  /* 🎤 v7.22 — DENTRO DE LA APK MANDA EL OÍDO DE ANDROID; en la web, el de siempre.
     Éste es todo el cambio: elegir la puerta. Lo de después —pintar lo dictado y
     mandárselo a Roberto— es el mismo camino para los dos. */
  if(OIDO.on){ oidoParar(); return; }        /* segundo toque = parar */
  if(oidoEmpezar()) return;
...
function robVidaUnGesto(){
  ...
  try{ robGiroDeMomento(emo); }catch(_){}
}

/* ══ 🔄 CUÁNDO SE GIRA, Y CUÁNDO NO — v7.57 ═══════════════════════════════════════════
   Rey preguntó "¿en qué momento se vira de espalda, con qué gestos y señales?" y al ver que
   no significaba nada (salía cada nueve gestos, sin más) lo aprobó CON SU LEY:
   *"que no interfiera con ninguna alarma ni aviso importante… debe ser en el momento, sin
   interrumpir nada importante. Me va a mostrar el gráfico pero DE FRENTE, hablándome todo
   el tiempo, notificándome las alarmas y los avisos y respondiéndome"*.
   Así que el giro NO es un adorno cada N gestos: pasa por MOTIVOS, y solo cuando de verdad
   no hay nada. Y de costado NO se gira: esa vista no existe y decir que sí sería mentirle. */
const ROB_GIRA_POR = {
  serio:    "cierras el día: se va de espaldas, como quien se marcha",
  siesta:   "se retira a esperar: se gira antes de apartarse",
  espera:   "en guardia y sin nada que hacer: mira lo que tiene detrás",
  aburrido: "mercado cerrado: se entretiene mirando atrás",
};
function robGiroDeMomento(emo){
  if(typeof Roberto === "undefined" || typeof Roberto.mirar !== "function") return;
  /* 🔒 SU LEY, PUNTO POR PUNTO: nada de girarse si hay algo que importe */
  const bloquea =
    (Date.now() - _robEvTs < 60000)                                  /* aviso o alarma reciente */
    || (typeof IA !== "undefined" && IA.busy)                        /* está pensando o respondiendo */
    || (typeof IA_TOOL_PEND !== "undefined" && (IA_TOOL_PEND||[]).some(function(p){ return p && p.estado==="pendiente"; }))  /* hay una tarjeta esperando su sí */
    || document.visibilityState !== "visible"                        /* Apex no está a la vista */
    || !!(($("#iaText")||{}).value||"").trim()                       /* Rey le está escribiendo */
    || (($("#robGlobo")||{}).style||{}).display === "block";         /* tiene la nubecita puesta */
  if(bloquea){ try{ Roberto.deFrente(); }catch(_){} return; }
  if(!ROB_GIRA_POR[emo]) return;
  /* y aun así, no cada vez: una de cada tres veces que toque, para que no sea un tic */
  window._vidaGiro = (window._vidaGiro || 0) + 1;
  if(window._vidaGiro % 3 !== 0) return;
  Roberto.mirar("espalda");
  /* vuelve solo; y si mientras tanto pasa algo, el cierre de abajo (poner/hablar) ya lo
     habrá puesto de frente y este setTimeout no molesta */
  setTimeout(function(){ try{ Roberto.deFrente(); }catch(_){} }, 2800);
}
...
function robVistazoTexto(){
  ...
  try{
    ...
    /* 📅 v7.62 — EL DOMINGO EXISTE. parseDias() numera lunes=1 … DOMINGO=7; getDay() numera
       DOMINGO=0 … sábado=6. Comparar el uno con el otro hacía que los domingos NINGÚN aviso
       encajara, y la nube le decía "no te queda ninguno" teniendo dos por delante. */
    const hoyDia = (ahora.getDay() === 0) ? 7 : ahora.getDay();
    ...
    pend = quedan.length;
    /* corta: el número y el próximo. Si no queda ninguno se dice en tres palabras, no en
       una frase larga — la nube se lee de un vistazo o no sirve de nada. */
    if(quedan.length) l.push("⏰ " + quedan.length + " aviso" + (quedan.length>1?"s":"") +
      " · " + quedan[0].hora + " " + String(quedan[0].tit||"").slice(0,22));
    else l.push("⏰ Nada más hoy");
  }catch(_){}
  /* 💰 el margen que le queda, de la cuenta más apretada */
  try{
    /* 💰 v7.62 — SOLO SI DE VERDAD VA APRETADA. Antes salía SIEMPRE, y una cuenta con todo su
       margen intacto no es noticia: solo alargaba la nube. `peor` ya mide eso (0 holgada …
       3 sin margen), así que se enseña de 2 para arriba, que es cuando le queda menos del
       30% de su colchón. */
    ...
    if(r && r.length){
      ...
      if((c.peor||0) >= 2){
        ...
        l.push("💰 " + String(c.alias).slice(0,16) + ": " + dia + " hoy · " + tot + " total");
      }
    }
  }catch(_){}
  /* 🤖 el Ejecutor */
  try{
    /* 🤖 v7.62 — ESTA LÍNEA MENTÍA SIEMPRE, DESDE QUE NACIÓ. Leía d.sw.on, d.live.vivo y
       live.pos: TRES CAMPOS QUE NO EXISTEN. Los de verdad son d.on, d.vivo y live.posiciones.
       Como los tres daban undefined, la nube decía "Ejecutor apagado · PC sin conexión" con
       el Ejecutor ENCENDIDO y la PC CONECTADA — justo el tipo de dato falso que Rey no
       perdona. Y ahora solo sale si hay ALGO QUE MIRAR: que esté apagado, que la PC no
       responda, o que haya posiciones vivas. Que todo esté bien no es una noticia. */
    const d = (typeof EJEC_CACHE!=="undefined" && EJEC_CACHE) ? EJEC_CACHE.d : null;
    if(d){
      const live = d.live || {};
      const on = !!d.on, vivo = !!d.vivo;
      const abiertas = Array.isArray(live.posiciones) ? live.posiciones.length : (+live.posiciones||0);
      if(!on)             l.push("🤖 Ejecutor apagado");
      else if(!vivo)      l.push("🤖 Ejecutor sin PC");
      else if(abiertas)   l.push("🤖 " + abiertas + " posición(es) viva(s)");
    }
  }catch(_){}
  ...
  try{
    /* 🔔 v7.62 — UNA SOLA CAMPANA: el texto de la señal ya viene con la suya y salía "🔔 🔔". */
    if(_VISTAZO_SENAL){
      /* se corta por la última palabra entera y sin dejar colgando un "Entrada:" a medias:
         un renglón cortado a mitad de palabra se lee peor que uno más corto. */
      const full = _VISTAZO_SENAL.replace(/^\s*🔔\s*/, "");
      let s = full;
      if(s.length > 40){ const c = s.lastIndexOf(" ", 40); s = s.slice(0, c > 24 ? c : 40); }
      s = s.replace(/[\s|:.,-]+$/, "");
      /* y si la última palabra era una ETIQUETA (en la señal venía seguida de ":"), se cae
         también: "…CORTO B Entrada" no dice nada, "…CORTO B" sí. */
      const resto = full.slice(s.length).replace(/^\s+/, "");
      if(resto.charAt(0) === ":") s = s.replace(/\s*\S+$/, "");
      s = s.replace(/[\s|:.,-]+$/, "");
      if(s) l.push("🔔 " + s);
    }
  }catch(_){}
